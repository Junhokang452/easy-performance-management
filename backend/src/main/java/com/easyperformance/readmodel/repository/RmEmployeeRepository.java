...
public interface RmEmployeeRepository extends JpaRepository<RmEmployee, UUID> {
    ...
    List<RmEmployee> findAllByTenantIdAndIdIn(UUID tenantId, Collection<UUID> ids);

    @Query("""
        select e from RmEmployee e
        where e.tenantId = :tenantId and upper(e.status) = 'ACTIVE'
          and (:query = '' or lower(e.name) like lower(concat('%', :query, '%'))
               or lower(e.employeeNo) like lower(concat('%', :query, '%')))
        """)
    Page<RmEmployee> searchActive(@Param("tenantId") UUID tenantId,
