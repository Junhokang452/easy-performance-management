/**
 * GradeBadge — 등급(S~D + UNRATED) Mantine UiBadge.
 *
...
export function GradeBadge({ grade, size = 'sm' }: Props): React.ReactNode {
  ...
  if (grade == null || grade === '') {
    return (
      <UiBadge color="gray" variant="outline" size={size}>
        —
      </UiBadge>
    );
  }
  ...
  return (
    <UiBadge color={color} variant={grade === 'UNRATED' ? 'outline' : 'filled'} size={size}>
      {label}
    </UiBadge>
  );
}
